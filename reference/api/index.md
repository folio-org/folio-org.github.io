  ...
  {%- for module in group[1].modules -%}
    ...
    {% for repo in site.data.config-apidocs %}
      {% if repo.name == module %}
        ...
        {%- capture urlConfig %}{{ urlAws }}/{{ theRepo.name }}/config-doc.json{% endcapture -%}
        {% break %}
      {% endif %}
    {% endfor %}
...
<p>
<a href="{{ urlSourceXrefLocal }}">Documentation</a> for {{ moduleId }}.
<a href="{{ urlConfig }}">API data</a>.
</p>
  {% endfor %}
{% endfor %}
...
The API documents are generated by the continuous integration (CI) task "[api-doc](/guides/api-doc)" for each repository during the "merge to main branch" phase or upon module release.
...
Similarly each row of a module's documentation table (e.g. [#mod-circulation-change-due-date](#mod-circulation-change-due-date)).
