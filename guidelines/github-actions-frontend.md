...
The workflows cannot be run sucessfully directly from the forked repository. There are organisational level secrets required to run CI steps such as `Run SonarCloud scan` and GA doesn't provide access to these secrets causing the workflow to fail.
...
The above steps can be only peformed by someone who has write access to the target repository, so the forked PR author should add a **reviewer** to the PR who can then merge the PR.
