...
def report_summary(json_apidocs, json_old_config, list_modules):
    ...
    LOGGER.info("difference between old config and new: %s",
        old_config_list.difference(new_config_list))
    LOGGER.info("difference between all potential and new config: %s",
        list_modules.difference(new_config_list))
    ...
    for mod in sorted(json_repos["repos"], key=itemgetter('name')):
        ...
        try:
            ...
            if is_api: #FIXME: repos.json sometimes has null property
                list_modules.add(mod["name"])
    LOGGER.info("Found %s repos that are API-related.", len(list_modules))
...
def main():
    ...
    report_summary(json_apidocs, json_old_config, list_modules)
    logging.shutdown()
    return exit_code
