...
Q: Are there any projects present at the moment, I could not find any in the list for the default cluster? How is a new project created?  
  + New Projects are available for authorized GitHub team members. Project deployed with Terraform already, including core backends, UI, secrets, Folio Helm Catalog etc.
...
Q: Who can do that? How is it done?  
  + It can be done in `Resources->Workloads` menu. Select desired Kubernetes Pod and use `+` and `-` buttons
